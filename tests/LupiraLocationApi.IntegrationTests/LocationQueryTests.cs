...
public sealed class LocationQueryTests(LocationApiTestFactory factory) : IntegrationTest(factory)
{
    ...
    public async Task Current_filters_by_device_and_lists_all_without_filter()
    {
        ...
        Assert.Equal(2, (await api.GetFromJsonAsync<List<CurrentFixDto>>("/location/current"))!.Count);
        var one = await api.GetFromJsonAsync<List<CurrentFixDto>>($"/location/current?deviceId={d1.Device.Id}");
        Assert.Single(one!);
    ...
    public async Task Track_defaults_to_last_day_when_no_range()
    {
        ...
        Assert.Single((await api.GetFromJsonAsync<List<TrackPointDto>>("/location/track"))!);
    }
    ...
    public async Task Thinned_track_collapses_density()
    {
        ...
        var raw = await api.GetFromJsonAsync<List<TrackPointDto>>($"/location/track?from={Q(baseTs.AddMinutes(-1))}&to={Q(baseTs.AddMinutes(10))}");
        var thinned = await api.GetFromJsonAsync<List<TrackPointDto>>($"/location/track/thinned?bucketSeconds=30&from={Q(baseTs.AddMinutes(-1))}&to={Q(baseTs.AddMinutes(10))}");
    ...
    public async Task Bbox_returns_only_fixes_in_the_rectangle()
    {
        ...
        var inBox = await api.GetFromJsonAsync<List<TrackPointDto>>($"/location/bbox?minLat=59.2&maxLat=59.4&minLon=17.9&maxLon=18.1&from={from}&to={to}");
        Assert.Single(inBox!);
        Assert.Equal(59.30, inBox![0].Lat, 2);

        var empty = await api.GetFromJsonAsync<List<TrackPointDto>>($"/location/bbox?minLat=10&maxLat=11&minLon=10&maxLon=11&from={from}&to={to}");
        Assert.Empty(empty!);
    }
    ...
    public async Task Stats_reports_a_plausible_distance()
    {
        ...
        var stats = await api.GetFromJsonAsync<TrackStatsDto>($"/location/stats?from={Q(baseTs.AddMinutes(-1))}&to={Q(baseTs.AddMinutes(10))}");
    ...
    public async Task Stats_empty_range_returns_zeroes()
    {
        ...
        var now = DateTimeOffset.UtcNow;
        var stats = await api.GetFromJsonAsync<TrackStatsDto>($"/location/stats?from={Q(now.AddHours(-1))}&to={Q(now)}");
        Assert.Equal(0, stats!.DistanceM);
        Assert.Equal(0, stats.SampleCount);
    ...
    public async Task Visits_trips_summary_empty_when_no_data()
    {
        ...
        Assert.Empty((await api.GetFromJsonAsync<List<LocationVisitDto>>($"/location/visits?from={from}&to={to}"))!);
        Assert.Empty((await api.GetFromJsonAsync<List<LocationTripDto>>($"/location/trips?from={from}&to={to}"))!);
        var summary = await api.GetFromJsonAsync<DailyLocationSummaryDto>($"/location/summary?date={day:yyyy-MM-dd}");
    ...
    public async Task Rollup_produces_visits_trips_and_summary()
    {
        ...
        var from = Q(baseTs.AddHours(-1));
        var to = Q(baseTs.AddHours(2));
        Assert.Equal(2, (await api.GetFromJsonAsync<List<LocationVisitDto>>($"/location/visits?from={from}&to={to}"))!.Count);
        Assert.True((await api.GetFromJsonAsync<List<LocationTripDto>>($"/location/trips?from={from}&to={to}"))!.Count >= 1);
        var summary = await api.GetFromJsonAsync<DailyLocationSummaryDto>($"/location/summary?date={day:yyyy-MM-dd}");
    ...
    public async Task At_returns_visit_when_inside_a_visit_window()
    {
        ...
        var at = await api.GetFromJsonAsync<PlaceLabelAtDto>($"/location/at?ts={Q(baseTs.AddMinutes(8))}");
    ...
    public async Task At_returns_nearest_fix_when_no_visit()
    {
        ...
        var at = await api.GetFromJsonAsync<PlaceLabelAtDto>($"/location/at?ts={Q(baseTs)}");
    ...
    public async Task At_returns_none_when_no_data()
    {
        var api = Factory.ApiClient("alice@x.test");
        await SetupDeviceAsync(api);
        var at = await api.GetFromJsonAsync<PlaceLabelAtDto>($"/location/at?ts={Q(DateTimeOffset.UtcNow)}");
    ...
    public async Task Purge_removes_raw_derived_and_current_in_range()
    {
        ...
        var from = Q(baseTs.AddHours(-1));
        var to = Q(baseTs.AddHours(2));
        var purge = await api.DeleteAsync($"/location?from={from}&to={to}");
        Assert.Equal(System.Net.HttpStatusCode.NoContent, purge.StatusCode);

        Assert.Empty((await api.GetFromJsonAsync<List<TrackPointDto>>($"/location/track?from={from}&to={to}"))!);
        Assert.Empty((await api.GetFromJsonAsync<List<LocationVisitDto>>($"/location/visits?from={from}&to={to}"))!);
