...
public sealed class DevicesTests(LocationApiTestFactory factory) : IntegrationTest(factory)
{
    ...
    public async Task Register_then_list_rename_retire_lifecycle()
    {
        ...
        Assert.Single((await api.GetFromJsonAsync<List<DeviceDto>>("/devices"))!);

        var rename = await api.PutAsJsonAsync($"/devices/{reg.Device.Id}", new RenameDeviceRequest { Label = "Trip Tracker" });
    ...
    {
        var api = Factory.ApiClient("alice@x.test");
        var resp = await api.PostAsJsonAsync("/devices", new RegisterDeviceRequest { Kind = DeviceKind.Phone, Label = "  " });
    ...
    public async Task Register_with_unknown_kind_is_400()
    {
        ...
        var resp = await api.PostAsJsonAsync("/devices", new { kind = "Toaster", label = "Kitchen" });
        Assert.Equal(HttpStatusCode.BadRequest, resp.StatusCode);
    }
    ...
    public async Task Devices_are_isolated_per_user()
    {
        ...
        var bList = (await b.GetFromJsonAsync<List<DeviceDto>>("/devices"))!;
    ...
    public async Task Another_user_cannot_rename_or_retire_my_device()
    {
        ...
        var b = Factory.ApiClient("b@x.test");
        Assert.Equal(HttpStatusCode.NotFound, (await b.PutAsJsonAsync($"/devices/{regA.Device.Id}", new RenameDeviceRequest { Label = "Hijack" })).StatusCode);
    ...
    public async Task Retired_device_key_can_no_longer_ingest()
    {
        ...
        Assert.Equal(HttpStatusCode.Accepted, (await PostNdjson(key, "/ingest/location", [Fix(1, DateTimeOffset.UtcNow.AddMinutes(-1), 59.3, 18.0)])).StatusCode);
        await api.DeleteAsync($"/devices/{reg.Device.Id}");
        Assert.Equal(HttpStatusCode.Unauthorized, (await PostNdjson(key, "/ingest/location", [Fix(2, DateTimeOffset.UtcNow, 59.3, 18.0)])).StatusCode);
    }
}
