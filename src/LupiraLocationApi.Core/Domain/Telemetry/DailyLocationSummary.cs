...
/// <summary>Per-day rollup of a principal's location (the <c>DailyHealthSummary</c> analogue for location). Deterministic
/// id from (principal, date, device) so re-running a day's rollup overwrites it. Lives in <c>health</c>; survives raw drop.</summary>
